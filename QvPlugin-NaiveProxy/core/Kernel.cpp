...
NaiveProxyKernel::NaiveProxyKernel() : PluginKernel()
{
    ...
    connect(&process, &QProcess::readyRead, [this]() { emit this->OnKernelLog(process.readAll()); });
...
bool NaiveProxyKernel::Start()
{
    const auto executablePath = NaiveProxyPluginInstance->GetSettings()["kernelPath"].toString();
    if (!QFile::exists(executablePath))
    {
        NaiveProxyPluginInstance->PluginErrorMessageBox(tr("Naive!"), tr("We cannot find your NaiveProxy kernel. Please configure it in the plugin settings."));
        return false;
    }
...
void NaiveProxyKernel::SetConnectionSettings(const QMap<KernelOptionFlags, QVariant> &options, const QJsonObject &settings)
{
    sni.clear();
    ...
    {
        emit OnKernelLog("warning: outbound protocol falled back to https");
        this->protocol = "https";
    }

    if (this->port <= 0 || this->port >= 65536)
    {
        emit OnKernelLog("warning: outbound port falled back to 443");
        this->port = 443;
    }
}
